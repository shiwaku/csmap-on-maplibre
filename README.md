...
レイヤー名は「地域名CS立体図（整備主体／解像度／年度）」の形式です。

- **整備主体**：元データ（航空レーザ測量成果・DEM）を整備した機関。shi-works が CS 立体図化したものも元データの機関を記載しています
- **解像度**：元 DEM のメッシュサイズ、または CS 立体図の地上画素寸法
- **年度**：元データの計測年度（西暦）。複数年度にわたるものは範囲で記載
- 公開資料で確認できなかった項目は「-」としています（2026-09 時点）

> **2026-09 配信元を `*.xsrv.jp` から `shi-works.com` に移行しました。** 旧 URL を参照している場合は [csmap-tiles の MIGRATION.md](https://github.com/shiwaku/csmap-tiles/blob/main/MIGRATION.md) の規則で書き換えてください（`xs489works.xsrv.jp` は 2026-09-30 停止）。
...
> **試験公開中のため、予告なく変更または公開を中止する場合があります。**
...
`https://shi-works.com/raster-tiles/` から配信している CS 立体図タイルは [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/deed.ja) で提供されます。
データセットごとの出典表示・原初データの利用規約・配信上の注意は、タイル専用リポジトリ
**[shiwaku/csmap-tiles](https://github.com/shiwaku/csmap-tiles)** を参照してください。
タイルの配信は個人運用で可用性を保証しません。URL の変更は同リポジトリで案内します。
